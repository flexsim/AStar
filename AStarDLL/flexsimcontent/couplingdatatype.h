...
#		if defined FLEXSIM_ENGINE_COMPILE
		...
		if (!branch)
			return;
		if (branch->size() < max(rank1, rank2) || rank1 <= 0 || rank2 <= 0)
			return;
		return parent->branch->swap(rank1, rank2);
#		else
		swapnoderanks(parent, rank1, index2 + 1);
#		endif
	}
...
public:
	...
	static TreeNode* NodePtrGetter(treenode x) { return x->objectAs(CouplingDataType)->partner(); }
	static void NodePtrAdder(treenode x, TreeNode* obj) { x->objectAs(CouplingDataType)->partner() = obj; }

	static TreeNode* StoredAttCouplingGetter(treenode x)
	{
		treenode partner = x->objectAs(CouplingDataType) ? x->objectAs(CouplingDataType)->partner().get() : 0;
		if (partner)
	...
	static void SdtSubNodeCouplingAdder(treenode x, T* obj)
	{
		...
		x->joinTo(obj->holder->subnodes.add()->addData(DATA_POINTERCOUPLING));
	}
	...
	static T* SdtSubSubNodeCouplingGetter(treenode x)
	{
		treenode partner = x->dataType == DATA_POINTERCOUPLING ? x->objectAs(CouplingDataType)->partner().get() : 0;
		if (partner)
			return partner->up->up->objectAs(T);
		return 0;
	}
	...
	static void SdtSubNodeAdder(treenode x, T* obj) { x->addSimpleData(obj, 0); }
	static void SdtSubNodeBindingAdder(treenode x, T* obj) { x->addSimpleData(obj, 1); }
	static void CouplingSdtSubNodeAdder(treenode x, T* obj) { x->addCouplingData(obj, 0); }
	static void CouplingSdtSubNodeBindingAdder(treenode x, T* obj) { x->addCouplingData(obj, 1); }
	static void ObjSubNodeAdder(treenode x, T* obj) { obj->holder->up = x->up; x->destroy(); }
	...
	static T* ObjCouplingGetter(treenode x)
	{
		treenode partner = x->dataType == DATA_POINTERCOUPLING ? x->object<CouplingDataType>()->partner().get() : 0;
		if (partner)
			return &o(T, partner->ownerObject);
		return 0;
	}
	static void ObjCouplingAdder(treenode x, T* obj)
	{
		AssertionMethod()(obj->holder != 0, "Adding object that is not bound to the tree");
		x->joinTo(obj->assertAttribute("stored", 0)->subnodes.add()->addData(DATA_POINTERCOUPLING));
	}
	...
	static T* ObjPortForwarder(treenode partner)
	{
		...
		if (partner->subnodes.length > 1) {
			...
			if (rank2->datatype == DATA_POINTERCOUPLING) {
				TreeNode* nextPartner = rank2->objectAs(CouplingDataType)->partner();
				if (nextPartner) {
			...
			if (upNode->datatype == DATA_POINTERCOUPLING) {
				TreeNode* nextPartner = upNode->objectAs(CouplingDataType)->partner();
				if (nextPartner) {
					return ObjPortForwarder(nextPartner);
				}
			}
		}
		return partner->ownerObject->objectAs(T);
	}
public:
	static T* ObjPortGetter(TreeNode * x)
	{
		treenode partner = x->dataType == DATA_POINTERCOUPLING ? x->objectAs(CouplingDataType)->partner().get() : 0;
		if (!partner->isValidNode()) return nullptr;
	...
	{
		AssertionMethod()(obj == nullptr || obj->holder != 0, "Adding object that is not bound to the tree");
		PortAdder(x, obj, obj ? obj->Nb_connectionsin : nullptr);
	}
	static void OutPortAdder(treenode x, T* obj)
	{
		AssertionMethod()(obj == nullptr || obj->holder != 0, "Adding object that is not bound to the tree");
		PortAdder(x, obj, obj ? obj->Nb_connectionsout : nullptr);
	}
	static void CenterPortAdder(treenode x, T* obj)
	{
		AssertionMethod()(obj == nullptr || obj->holder != 0, "Adding object that is not bound to the tree");
		PortAdder(x, obj, obj ? obj->Nb_connectionscenter : nullptr);
	}
